...
----------------------------------------------------------------------------------------------------------
  ...
  v1.0.1         190801         EtcFly          fix bug which can cause RT_ASSERT fail (because incorrect memory free)
**********************************************************************************************************/
#include <rtthread.h>
#include <rtdevice.h>
#include <board.h>
...
int threadManager_init(void)
{
    ...
    rt_list_init(&s_threadM.delList);
...
void threadM_update(threadUnit_Type *unit)
{
    ...
    if (s_threadM.b_reboot)
    {
        if (!rt_list_isempty(&s_threadM.delList))
        {
            rt_list_t *l;
            threadUnit_Type *threadUnit;

            l = &s_threadM.delList;
            l = l->next;

            while  (&s_threadM.delList != l)
            {
                threadUnit = rt_list_entry(l, threadUnit_Type, list);
                if (threadUnit == unit) {

                    rt_list_remove(l);
                    //old thread resource release 
                    if (!rt_list_isempty(&threadUnit->memList)) {
                        threadM_memFree(&threadUnit->memList);
                    }

                    if (!rt_list_isempty(&threadUnit->ipcList)) {
                        threadM_ipcFree(&threadUnit->ipcList);
                    }

                    threadM_unregister(threadUnit);  
					rt_kprintf("free memory threadUnit!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\r\n");
                    break;         
                }
...
static rt_err_t threadM_recoverThread(threadUnit_Type *unit, threadInfo_Type *info)
{
    ...
    if (RT_NULL != pthread) {
        if (rt_thread_startup(pthread) == RT_EOK) {
            ...
            s_threadM.b_reboot = 1;
            rt_list_remove(&unit->list);
            rt_list_insert_after(&s_threadM.delList, &unit->list);
...
RT_USED static rt_base_t thread_listCount(rt_list_t *l)
{
    rt_base_t count = 0;
    if (rt_list_isempty(l)) {
        return count;
